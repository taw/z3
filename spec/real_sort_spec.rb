describe Z3::RealSort do
  it "can instantiate constants" do
    expect(subject.from_const(0).inspect).to eq("Value<0.0 :: Real>")
    expect(subject.from_const(42).inspect).to eq("Value<42.0 :: Real>")
    expect(subject.from_const(1_000_000_000_000).inspect).to eq("Value<1000000000000.0 :: Real>")
    expect(subject.from_const(-1_000_000_000_000).inspect).to eq("Value<(- 1000000000000.0) :: Real>")
    expect(subject.from_const(0.0).inspect).to eq("Value<0.0 :: Real>")
    expect(subject.from_const(-0.0).inspect).to eq("Value<0.0 :: Real>")
    expect(subject.from_const(3.14).inspect).to eq("Value<(/ 157.0 50.0) :: Real>")
    expect(subject.from_const(-3.14).inspect).to eq("Value<(- (/ 157.0 50.0)) :: Real>")
  end
